    ...
    <v-card>
        ...
        <v-row>
          ...
          <v-col>
            <v-btn color="orange-lighten-1"
                   variant="text"
                   @click.stop="active='setup'">Наладочная
            </v-btn>

            <operation-create :operation="setupPrice"
                              :visible="active==='setup'"
                              @hide="active=-1"
                              @save="save($event)"
            />
          </v-col>
          <v-col>
            <v-btn color="orange-lighten-1"
                   variant="text"
                   @click.stop="active='tech'">Технолог
            </v-btn>

            <operation-create :operation="techPrice"
                              :visible="active==='tech'"
                              @hide="active=-1"
                              @save="save($event)"
            />
      ...
      <v-list>
        ...
        <v-list-item :title="'...'">
          ...
          <operation-create :operation="{paymentPerHour:{currency: 'RUB'}}"
                            :visible="active==='new'"
                            @hide="active=-1"
                            @save="save($event)"
          />
