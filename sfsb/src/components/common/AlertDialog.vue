<template lang="pug">
    v-dialog(v-model="isShow", width="500")
        v-card(color="orange")
            v-card-title
                span.headline {{props.title || 'Вы не являетесь автором данной заявки'}}
            v-card-text  {{props.text || 'При изменении данных данной заявки Вы автоматически станете ее автором.'}}
            v-card-actions
                v-spacer
